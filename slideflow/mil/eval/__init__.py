...
from .._params import (
    TrainerConfig, ModelConfigCLAM, TrainerConfigCLAM, ModelConfigPORPOISE
)
...
def predict_surv(
    ...
    assert isinstance(config.model_config, ModelConfigPORPOISE)
...
def eval_mil_surv(
    ...
    *,
    ...
    df_dict['y_pred0'] = y_pred
    df = pd.DataFrame(df_dict)
    pred_out = join(model_dir, 'predictions.csv')
