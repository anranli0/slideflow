...
class MIL_Attention_FC_surv(nn.Module):
    ...
    size_dict_path = {"small": [1024, 512, 256], "big": [1024, 512, 384]}
    size_dict_omic = {'small': [256, 256]}

    def __init__(
        self, 
        size_arg: Union[str, List[int]] = "small",
        ...
        ### Deep Sets Architecture Construction
        size = self.size_dict_path[size_arg] if isinstance(size_arg, str) else size_arg
        self.size = size
    ...
    def forward(self, x_path, x_omic=None):
        ...
        A_raw = A 
        A = F.softmax(A, dim=1) 
        h_path = torch.matmul(A.squeeze(), h_path.squeeze()) #torch.mm
...
class MIL_Cluster_FC_surv(nn.Module):
    ...
    def forward(self, x_path, cluster_id, x_omic=None):
